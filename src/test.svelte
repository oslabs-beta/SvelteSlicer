<script defer>
    ...
    $: newComponent = JSON.parse(JSON.stringify($snapshots[I].data[parent]));
    ...
    const height = document.body.clientHeight;
       ...
       function trimTree(tree){
        ...
        svg = d3.select('#chart')
           ...
           //construct root node
           trimTree(newComponent);

           console.log('trimeTree value',newComponent);
           root = d3.hierarchy(newComponent, function(d){
            ...
            let activeNode = treeData.descendants();
               ...
               let nodeEnter = node
                      ...
                      .attr('transform',function(d){
                          ...
                          let arr
                          if(d.parent){
                          arr = d.parent.data.children;
                          //console.log('arr',arr)
                          }
                          
                        //   let nodecounts=0
                        //   arr.forEach(item=>{
                        //       if(item.active) nodecounts++
                        //   })
                        //   console.log('countnode',nodecounts)
                          if(arr !==undefined){
                            //   root.x0 = root.x0/arr.length
                            return update(root)
                ...
                nodeUpdate
                  ...
                  .attr('transform',function(d){
                    //  console.log('d??',d)
                    //  if(d.data.parent){
                    //      let count = d.parent.children
                    //  console.log('x--',count.length)
                    
                    // return 'translate(' +d.x/count.length+ ', ' +d.y+ ')'
                    //  }
                    //  else{
